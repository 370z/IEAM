        ...
        >
          ...
          <template v-slot:[`item.iron`]="{ item }">
            <span :style="`color: ${item.iron == 0 ? '#FFC83B' : '#30C03E'}`">
              {{ item.iron == 0 ? "รอการอนุมัติ" : "อนุมัติเรียบร้อย" }}
            </span>
          </template>
          <template v-slot:[`item.actions`]="{ item }">
            <div v-if="item.iron == 0">
              <v-btn color="success" rounded x-small fab elevation="false">
                <v-icon @click="dialog = true"> mdi-check </v-icon></v-btn
              >
              <v-btn class="ml-2" color="error" rounded x-small fab elevation="false">
                <v-icon @click="del(item)">mdi-close </v-icon></v-btn
              >
            </div>
            <div v-if="item.iron == 1">
